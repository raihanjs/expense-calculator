...
export default function ExpenseTable({
  ...
  let expenseStatement = statement.filter((item) => item.type === "Expense");

  // Sort Income
  expenseStatement.sort((a, b) => {
    if (sort === 1) {
  ...
  // Filter Income
  if (filter.length > 0) {
    expenseStatement = expenseStatement.filter((expense) =>
      filter.includes(expense.category)
    );
  }
